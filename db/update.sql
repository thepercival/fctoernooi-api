...
insert into associations( name ) ( select concat(a.name,'-',l.id) from leagues l join associations a on a.id = l.associationid where exists( select * from leagues lsub where lsub.id < l.id and lsub.associationid = l.associationid ) );
...
update competitors c join associations a on a.id = c.associationid join leagues l on l.associationid = a.id join competitions comp on comp.leagueid = l.id join tournaments t on t.competitionid = comp.id set c.tournamentId = t.id;
...
INSERT INTO gameAmountConfigs ( amount, nrOfGamesPerPlaceMixed, roundNumberId, competitionSportId )(SELECT pc.nrOfHeadtohead, 0, rn.id, (select id from competitionSports where competitionId = rn.competitionId ) from roundNumbers rn join planningConfigs pc on rn.planningConfigId = pc.id);
-- parent is null
INSERT INTO scoreConfigs ( direction, maximum, enabled, parentId, competitionSportId, roundId )(SELECT ssc.direction, ssc.maximum, ssc.enabled, null, (select id from competitionSports where competitionId = rn.competitionId ), r.id from rounds r join roundNumbers rn on r.numberId = rn.id join sportscoreconfigs ssc on ssc.roundnumberid = rn.id and ssc.parentid is null);
-- parent is not null
INSERT INTO scoreConfigs ( direction, maximum, enabled, parentId, competitionSportId, roundId )(SELECT ssc.direction, ssc.maximum, ssc.enabled, sc.id, sc.competitionSportId, sc.roundId from scoreConfigs sc join rounds r on r.id = sc.roundId join roundNumbers rn on r.numberId = rn.id join sportscoreconfigs ssc on ssc.roundnumberid = rn.id and ssc.parentid is not null);
-- CHECK INSERT INTO againstQualifyConfigs
INSERT INTO againstQualifyConfigs ( winPoints, drawPoints, winPointsExt, drawPointsExt, losePointsExt, pointsCalculation, competitionSportId, roundId )(SELECT sc.winPoints, sc.drawPoints, sc.winPointsExt, sc.drawPointsExt, sc.losePointsExt, sc.pointsCalculation, (select id from competitionSports where competitionId = rn.competitionId ), r.id from rounds r join roundNumbers rn on r.numberId = rn.id join sportconfigs sc where sc.competitionid = rn.competitionid and rn.number = 1);
INSERT INTO againstGames (id, pouleid, resourcebatch, state, startDateTime, refereeId, placerefereeId, fieldId, competitionSportId, gameRoundNumber ) SELECT id, pouleid, resourcebatch, state, startDateTime, refereeId, placerefereeId, fieldId, (select id from competitionSports where competitionId = ( select rn.competitionId from poules p join rounds r on r.id = p.roundid join roundNumbers rn on rn.id = r.numberid where p.id = games.pouleid ) ), 0 from games;

INSERT INTO againstGamePlaces (side, placeId, gameId)(SELECT if(homeAway,1,2), placeId, gameId from gameplaces);
INSERT INTO againstScores (phase, number, home, away, gameId) (SELECT phase, number, home, away, gameId from gamescores);
